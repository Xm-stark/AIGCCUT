...
def create_video_mashup(video_clips, add_transitions=True):
    ...
    # 2. 添加视频片段
    # 注意：有转场时需要让相邻片段在时间轴上重叠 transition_duration，
    # 否则切点会出现一帧黑屏/空轨。
    current_time = 0
    ...
    for i, clip in enumerate(video_clips):
        ...
        transition_duration = 0.5 if transition else 0.0
        target_start = max(0, current_time - transition_duration)
        ...
        # 有转场时，下一段起点要提前 transition_duration，避免黑帧
...
def create_image_slideshow(image_urls, image_duration=3, transition="fade_in"):
    ...
    # 2. 依次添加图片
    # 注意：转场时需要重叠 transition_duration，否则会出现黑帧
    current_time = 0
    for i, image_url in enumerate(image_urls):
        transition_duration = 0.5 if i > 0 and transition else 0.0
        target_start = max(0, current_time - transition_duration)
        requests.post(f"{BASE_URL}/add_image", json={
            "draft_id": draft_id,
            "image_url": image_url,
            "start": target_start,
            "end": target_start + image_duration,
            "transition": transition if i > 0 else None,
            "transition_duration": transition_duration
        })
        current_time += image_duration - transition_duration
...
## 榛樿閰嶇疆锛堟帹鑽愶級
1. 瀛楀箷锛氶粍搴曢粦瀛楋紝瀛楀彿 9锛屽簳閮ㄥ眳涓紝璺濆簳閮?60px锛屽瓧骞曚笌鍙ｆ挱闊抽涓ユ牸鍚屾銆?
2. 鍒嗛暅鏉愭枡锛氭瘡娈垫敹鍒嗘椂闂? > 5s锛堟渶鍚庝竴娈靛彲鏍规嵁鍓╀綑鏃堕棿寰皟锛夈€?
3. BGM锛氭瘮鍙ｆ挱浣庝簬 2dB锛屼笉鎺╄鍙ｆ挱銆?
4. 姘村嵃锛氬悎鎴愬悗娣诲姞姘村嵃锛岄粯璁ゅ彲缁欏嚭鍙樺崟浣嶇疆锛堟瘮濡傚乏涓婅锛夈€?
5. 杈撳嚭锛氳緭鍑哄埌 `D:\\cutcup\\cutcup-pipeline\\remotion\\output\\final`锛屽懡鍚嶄负鏃ユ湡+搴忓彿锛屼緥濡? `3鏈?9鏃?锛?1锛夈€?
6. 杈呭姪 skills锛氫娇鐢? `D:\\cutcup\\cutcup-pipeline\\create-pr\\remotion\\remotion\\rules` 涓嬬殑 skills 辅助运行锛岄亣閿欒嚜鎴戣皟鏁村苟浼樺寲鑴氭湰銆?
...
**原因**：转场时间轴没有重叠，导致切点出现短暂空轨。  
**解决**：让相邻片段在时间轴上重叠 `transition_duration`。

```python
# 正确：有转场时让片段重叠 transition_duration
transition_duration = 0.5
target_start = max(0, current_time - transition_duration)
